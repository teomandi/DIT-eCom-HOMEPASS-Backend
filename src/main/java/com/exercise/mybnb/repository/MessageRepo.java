...
public interface MessageRepo extends JpaRepository<Message, Integer> {
    ...
    @Query("SELECT m FROM Message m WHERE m.hoster=?1 and ((m.reciever=?1 and m.sender=?2) or (m.reciever=?2 and m.sender=?1)) order by  m.createdAt DESC ")
